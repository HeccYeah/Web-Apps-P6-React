...
async function getData()
{
    ...
    let response1 = await fetch(`http://localhost:3001/users/${id}`)
    let respJson1 = await response1.json();
    let respObj1 = respJson1[0];
    let role = respObj1.Role;
    ...
    else if (role == "Advisor")
    {
        ...
        let adResponse = await fetch(`http://localhost:3001/prof/${id}`)
        let adRespJson = await adResponse.json();
        let adRespObj = adRespJson[0];

        let advisees = adRespObj.advisees;
        ...
        advisees.forEach(advisee => {
            ...
            stuOption.value = advisee.s_id;
            stuOption.textContent = advisee.s_id + " " + advisee.first_name + " " + advisee.last_name;
